...
export function ManageCustomerTypes() {
  const { t } = useLanguage();
  ...
  const openDialog = (type: CustomerType | null = null) => {
    setEditingType(type);
  ...
  const onSubmit = async (data: TypeFormValues) => {
    ...
    if (editingType) {
        const index = currentTypes.findIndex((t: CustomerType) => t.id === editingType.id);
        if (index > -1) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
            ...
            <CardDescription>Create and manage your customer types.</CardDescription>
        </div>
        ...
        <Table>
          ...
          <TableBody>
            {types.map((type) => (
             ...
             {types.length === 0 && (
                <TableRow>
                    <TableCell colSpan={3} className="text-center h-24">No customer types found.</TableCell>
                </TableRow>
            )}
          </TableBody>
        </Table>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editingType ? 'Edit' : 'Add'} Customer Type</DialogTitle>
            <DialogDescription>
              {editingType ? 'Update the details for this customer type.' : 'Create a new category for your customers.'}
            </DialogDescription>
          </DialogHeader>
           <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-4">
               <FormField
                ...
                render={({ field }) => (
                    ...
                    <FormControl>
                        <Input placeholder="e.g., Retailer, Wholesaler" {...field} />
                    </FormControl>
                    <FormMessage />
                    </FormItem>
                )}
